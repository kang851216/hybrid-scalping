...
dates_pool=[]
for m in range(1,4):
    for d in [1,4,7,10,13,16,19,22,25,28,31]:
...
for sd in TEST_DATES:
    ...
    total+=1
    m='*' if nr>bh else ' '
    print(f'{m}{total:>3} {sd} {pick["symbol"]:<6} H={nr:>+7.1f}% BH={bh:>+7.1f}% {w}')

if diffs:
    print(f'---')
    print(f'2026 #1 x3mo x$5K: {wins}/{total} wins ({wins/total*100:.0f}%) | Avg H={np.mean(diffs):+.1f}%')
