"""20 dates x Top 5 stocks x $1,000 each — Portfolio validation"""
...
dates_pool=[]
for y in [2025,2026]:
    for m in range(1,min(13,7 if y==2026 else 13)):
        for d in [3,8,13,18,23,28]:
            try:pd.Timestamp(f'{y}-{m:02d}-{d:02d}');dates_pool.append(f'{y}-{m:02d}-{d:02d}')
            except:pass
...
for sd in TEST_DATES:
    ed=(pd.Timestamp(sd)+pd.Timedelta(days=180)).strftime('%Y-%m-%d')
    picks=screen_top5(sd)
    if len(picks)<3:continue
    port_h=0;port_b=0;n_stocks=0
    for pick in picks:
        nr,bh=hybrid_5d(pick['symbol'],sd,ed)
        if nr is not None:port_h+=nr;port_b+=bh;n_stocks+=1
    if n_stocks==0:continue
    port_h/=n_stocks;port_b/=n_stocks
    ...
    stocks_str=','.join(p['symbol'] for p in picks[:3])
    print(f'{m}{total:>3} {sd} ({n_stocks}stk) {stocks_str} H={port_h:>+6.1f}% BH={port_b:>+6.1f}% {w}')

avg_h=np.mean(grand_h);avg_b=np.mean(grand_b)
print(f'---')
print(f'Top 5 x $1,000: {wins}/{total} wins ({wins/total*100:.0f}%) | Avg H={avg_h:+.1f}% | Avg BH={avg_b:+.1f}%')
